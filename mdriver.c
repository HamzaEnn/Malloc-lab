...
static block_t heap[MAXHEAP]; /* heap to parse output of heapcheck */
...
static void init_trace(trace_t *trace);
...
static int parse_heap(char *file, block_t *heap, int max_blocks, int tracenum,
                      int opnum);
...
int main(int argc, char **argv) {
   ...
   */
  while ((c = getopt(argc, argv, "f:t:hvVgald")) != EOF) {
    switch (c) {
  ...
  if (errors == 0) {
    ...
    if (avg_mm_throughput > MAX_THRUPUT) {
      p2 = (double)(1.0 - UTIL_WEIGHT);
    } else {
      p2 = ((double)(1.0 - UTIL_WEIGHT)) * (avg_mm_throughput / MAX_THRUPUT);
    }

    printf("Avg throughput: %.4fKops\n", avg_mm_throughput / 1000);
...
static trace_t *read_trace(char *tracedir, char *filename) {
  ...
  if ((trace->allocated = (int *)malloc(trace->num_ids * sizeof(int))) == NULL)
    unix_error("malloc 5 failed in read_trace");
  /* initialize allocated to 0 */
  for (i = 0; i < trace->num_ids; i++)
    trace->allocated[i] = 0;

  if ((trace->check = (int *)malloc(trace->num_ids * sizeof(int))) == NULL)
    unix_error("malloc 6 failed in read_trace");
...
static void init_trace(trace_t *trace) {
  ...
  memset(trace->allocated, 0, trace->num_ids * sizeof(int));
  memset(trace->check, 0, trace->num_ids * sizeof(int));
}
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case REALLOC: /* mm_realloc */
      ...
      if (size == 0 && newp != NULL) {
        malloc_error(tracenum, i, "mm_realloc failed with size 0.");
        return 0;
      }
      if (size == 0 && newp == NULL) { /* same as free */
    ...
    case HEAPCHECK:
      ...
      //   fd = open(tmpnam(tmpfile),O_RDWR|O_CREAT,S_IRWXU);
      fd = mkstemp(tmpfile);
      if (fd < 0)
        unix_error("Could not create temporary file for heapcheck");
      fflush(stdout);
      stdoutold = dup(1);
      dup2(fd, 1);
      ...
      /* output student's heapcheck debug info */
      if (dump)
        mm_heapcheck();
      ...
      for (j = 0; j < heapsize; j++) {
        ...
        if (((request_id) > trace->num_ids) ||
            (trace->allocated[request_id] == 0)) {
          sprintf(msg, "Heapcheck : %d is not a valid request_id", request_id);
        ...
        if (trace->allocated[request_id] == 2) {
          sprintf(msg, "Heapcheck : Outputted block %d has been freed",
                  request_id);
          malloc_error(tracenum, i, msg);
          expected_blocks(trace);
          outputted_blocks(heap, heapsize);
        ...
        if (trace->check[request_id] == 1) {
          sprintf(msg, "Heapcheck : Block %d has been outputted twice",
                  request_id);
          malloc_error(tracenum, i, msg);
          expected_blocks(trace);
          outputted_blocks(heap, heapsize);
        ...
        if (trace->block_sizes[request_id] > payload) {
          sprintf(msg,
                  "Heapcheck : Block %d has an incorrect block size of %d, "
                  "expecting %d or larger",
                  request_id, payload, trace->block_sizes[request_id]);
          malloc_error(tracenum, i, msg);
          expected_blocks(trace);
          outputted_blocks(heap, heapsize);
          return 0;
      ...
      for (j = 0; j < trace->num_ids; j++)
        if ((trace->allocated[j] == 1) && (trace->check[j] == 0)) {
          sprintf(msg, "Heapcheck : Block %d not outputted by heapcheck", j);
          malloc_error(tracenum, i, msg);
          expected_blocks(trace);
          outputted_blocks(heap, heapsize);
          return 0;
    ...
    case FREE: /* mm_free */
      ...
      oldsize = trace->block_sizes[index];
      /* Check and make sure the data is uncorrupted */
      for (j = 0; j < oldsize; j++) {
        if (p[j] != (char)(index & 0xff)) {
          malloc_error(tracenum, i, "Corrupted data found when freeing.");
          return 0;
        }
      }
...
static double eval_mm_util(trace_t *trace, int tracenum, range_t **ranges) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case REALLOC: /* mm_realloc */
      ...
      if (newsize == 0 && newp != NULL)
        app_error("mm_realloc failed with size 0 in eval_mm_util");

      if (newsize == 0) { /* same as free */
        total_size -= oldsize;
        trace->block_sizes[index] = 0;
        break;
      }
...
static void eval_mm_speed(void *ptr) {
  ...
  for (i = 0; i < trace->num_ops; i++)
    ...
    case REALLOC: /* mm_realloc */
      ...
      oldp = trace->blocks[index];
      newp = mm_realloc(oldp, newsize);
      if (newsize == 0 && newp == NULL) { /* same as free */
...
/*
 * parse_heapcheck - Parse the output of the heapcheck into an internal data
 * structure returns -1 on error, 0 on success
 */
static int parse_heap(char *file, block_t *heap, int max_blocks, int tracenum,
                      int opnum) {
  FILE *fp;
  ...
  fp = fopen(file, "r");
  if (fp == NULL)
    unix_error("Reading from temporary file failed");
...
static void expected_blocks(trace_t *trace) {
  ...
  printf("Expected Output for HeapCheck:\n");
  for (j = 0; j < trace->num_ids; j++)
    if (trace->allocated[j] == 1) {
      active++;
      printf("$BLOCK %d %d\n", j, trace->block_sizes[j]);
    }
  printf("Number of Actual Active Blocks = %d\n", active);
}

static void outputted_blocks(block_t *heap, int heapsize) {
  int i;
  printf("Output produced by mm_heapcheck\n");
  for (i = 0; i < heapsize; i++) {
    printf("$BLOCK %d %d\n", heap[i].request_id, heap[i].payload);
  }
  printf("Number of Outputted Active Blocks = %d\n", heapsize);
}
...
static void usage(void) {
  ...
  fprintf(stderr, "\t-d         Dump output of mm_heapcheck.\n");
}
