	...
	size = trace->ops[i].size;
	  ...
	  for(j=0;j<heapsize;j++) {
	    ...
	    if (trace->allocated[request_id] == 2) { 
	      sprintf(msg,"Heapcheck : Outputted block %d has been freed",request_id);
	      malloc_error(tracenum,i,msg);
	      expected_blocks(trace);
	      outputted_blocks(heap,heapsize);
	      return 0;
	    ...
	    if (trace->check[request_id] == 1) { 
	      sprintf(msg,"Heapcheck : Block %d has been outputted twice",request_id);
	      malloc_error(tracenum,i,msg);
	      expected_blocks(trace);
	      outputted_blocks(heap,heapsize);
	      return 0;
	    ...
	    if (trace->block_sizes[request_id] > payload) { 
	      sprintf(msg,"Heapcheck : Block %d has an incorrect block size of %d, expecting %d or larger",request_id,payload,trace->block_sizes[request_id]);
	      malloc_error(tracenum,i,msg);
	      expected_blocks(trace);
	      outputted_blocks(heap,heapsize);
	      return 0;
